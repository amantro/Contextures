...
class INWJ(nn.Module):
    ...
    def forward(self, x: torch.Tensor, a: torch.Tensor,
        ...
        first_term = -torch.log(diagonal_elements).sum() / (N * r)
    
        # Second term: 1/(B²r) * sum_i sum_j sum_m K(x_i, a_{j,m})
        # Sum over all elements in the tensor
        second_term = k_xa.sum() / (N * N * r)
    
        # We just return the above INWJ loss which is the sum of first and second terms.
       
        if reduction == "mean":
            INWJ_loss = first_term + second_term
        elif reduction == "none": # What do I return here?
            INWJ_loss = -torch.log(diagonal_elements).sum(dim=1)/r + k_xa.sum(dim=(1,2))/(N * r) # Shape: (N,)
        else:
            raise ValueError("Reduction must be 'mean' or 'none'.")
        
        # Prepare loss dictionary for verbose information
        loss_dict = {
            'Info_NWJ_loss': torch.mean(INWJ_loss).item(),
            'mean of kernel_values': torch.mean(k_xa).item()
        }
        return INWJ_loss, loss_dict